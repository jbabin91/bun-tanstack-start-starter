  ...
  return (
    ...
    value.every(
      (todo) =>
        typeof todo === 'object' &&
        todo !== null &&
        typeof (todo as { id?: unknown }).id === 'number' &&
        typeof (todo as { name?: unknown }).name === 'string',
    )
  );
}
...
async function readTodos(): Promise<Todo[]> {
  const fileContents = await fs.promises
    .readFile(TODOS_FILE, 'utf8')
    .catch(() => JSON.stringify(defaultTodos, null, 2));

  const parsed: unknown = JSON.parse(fileContents);
  return isTodoArray(parsed) ? parsed : [...defaultTodos];
}
