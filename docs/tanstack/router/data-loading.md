...
> [!TIP]
> Using TanStack Start? Prefer server-side data access via server functions for DB/env access and redirects. See `../start/server-functions.md`. Use route `loader`s to orchestrate calls and return typed data to components.
...
> [!TIP]
> Framework-level error boundaries and server-rendering considerations are covered in Start’s docs: `../start/error-boundaries.md`. Use Router’s `errorComponent` for route-scoped UI handling and Start error boundaries for application-wide error UX.
...
Passing down global context to your router is great, but what if you want to provide context that is specific to a route? This is where the `beforeLoad` option comes in. The `beforeLoad` option is a function that runs right before attempting to load a route and receives the same parameters as `loader`. Beyond its ability to redirect potential matches, block loader requests, etc, it can also return an object that will be merged into the route's context. This context will be available to all the route's child routes. The resulting context will be available to the route's `loader` function.

Let's take a look at an example where we inject some data into our route context via the `beforeLoad` option:
